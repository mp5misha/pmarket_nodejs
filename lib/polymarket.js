...
// `retries` defaults to the patient RETRY_LIMIT (3, with growing backoff) —
// mirrors server/src/polymarket.js's getJson. Pass a lower value (see
// fetchMarketBySlug) for a latency-sensitive, user-initiated call that
// should fail fast instead of making someone watch a spinner through
// several retries' worth of backoff — or worse, outlive a serverless
// function's execution budget.
async function getJson(url, params = {}, { retries = RETRY_LIMIT } = {}) {
  const u = new URL(url);
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    ...
    try {
      ...
      if (res.status === 429) {
        if (attempt < retries - 1) await sleep(RETRY_BACKOFF_MS * 2 ** attempt);
        continue;
      }
      ...
      lastErr = err;
      // No point sleeping out a backoff window after the last attempt —
      // there's nothing left to retry into.
      if (attempt < retries - 1) await sleep(RETRY_BACKOFF_MS * 2 ** attempt);
    } finally {
      // Must run on every path (success, HTTP error, thrown/rejected
      // fetch) — previously only cleared after a successful fetch, so any
      // failure leaked a REQUEST_TIMEOUT_MS-long dangling timer.
      clearTimeout(timer);
    }
  }
  throw new Error(`GET ${url} failed after ${retries} attempt(s): ${lastErr}`);
}
...
export async function fetchMarketBySlug(slug, { retries } = {}) {
  const data = await getJson(`${GAMMA_BASE}/markets`, { slug, limit: 1, include_tag: "true" }, { retries });
 ...
 * lib/db.js's getSyncCursor/setSyncCursor) so a fresh run continues into
 * unseen markets instead of re-fetching the same top-of-list results every
 * time. Deliberately excludes batchSize/history/interval, which change how
 * a page is processed but not which markets match. */
