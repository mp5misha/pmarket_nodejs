...
-- Mirrors server/migrations/011_analysis_kind_and_fair_prob.sql — see that
-- file's comment for what these are. ALTER...ADD COLUMN IF NOT EXISTS (not
-- baked into the CREATE TABLE above) for the same reason every other
-- post-launch column here uses it: on a database created before this
-- column existed, CREATE TABLE IF NOT EXISTS is a no-op.
ALTER TABLE analyses ADD COLUMN IF NOT EXISTS kind TEXT NOT NULL DEFAULT 'market';
ALTER TABLE analyses ADD COLUMN IF NOT EXISTS fair_prob_yes DOUBLE PRECISION;
CREATE INDEX IF NOT EXISTS idx_analyses_slug_kind ON analyses(market_slug, kind);
...
-- Mirrors server/src/db.js's whale_top_traders (see migration 012's comment
-- for why this is a separate table from whale_positions).
...
const LATEST_ANALYSIS_SELECT = `
  (SELECT result_text FROM analyses WHERE analyses.market_slug = markets.slug AND analyses.kind = 'market'
     ORDER BY created_at DESC LIMIT 1) AS last_analysis_text,
  (SELECT created_at FROM analyses WHERE analyses.market_slug = markets.slug AND analyses.kind = 'market'
  ...
  -- re-state FAIR_PROBABILITY_YES (see lib/deepseek.js), and without this
  -- it'd otherwise blank out a perfectly good estimate from the original run.
  (SELECT fair_prob_yes FROM analyses WHERE analyses.market_slug = markets.slug AND analyses.kind = 'market'
     AND fair_prob_yes IS NOT NULL ORDER BY created_at DESC LIMIT 1) AS fair_prob_yes
`;
...
// AI analysis history + follow-up threads (Phase 3/4 parity). `kind`
// distinguishes a regular market analysis ('market', the default) from an
// "AI analysis of Whales activity" one ('whales') — mirrors
// server/src/db.js's createAnalysis/listAnalysesForMarket.
export async function createAnalysis(
  ...
  const { rows } = await pool.query(
    `INSERT INTO analyses (market_slug, parent_analysis_id, prompt_text, result_text, model_name, tokens_used, kind, fair_prob_yes)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
    [marketSlug, parentAnalysisId, promptText, resultText, modelName, tokensUsed, kind, fairProbYes]
  );
  return rows[0];
}
...
/** Overwrites one analysis row's fair_prob_yes — mirrors server/src/db.js's
 * updateAnalysisFairProb, used by the "Parse the AI response" button. */
export async function updateAnalysisFairProb(pool, id, fairProbYes) {
  const { rows } = await pool.query(
    "UPDATE analyses SET fair_prob_yes = $1 WHERE id = $2 RETURNING *",
    [fairProbYes, id]
  );
  return rows[0] ?? null;
}

export async function listAnalysesForMarket(pool, marketSlug, kind = "market") {
  const { rows } = await pool.query(
    "SELECT * FROM analyses WHERE market_slug = $1 AND kind = $2 ORDER BY created_at DESC",
    [marketSlug, kind]
  );
  return rows;
}
...
// market_current_price/market_no_price/market_resolution_date (via a LEFT
// JOIN — a trade must survive even if its market was since deleted from the
// local catalog) mirror server/src/db.js's listTrades, powering the My
// Trades grid's live "Current price"/"Position value"/"P&L"/"Market end"
// columns.
export async function listTrades(pool, { status, marketSlug } = {}) {
  ...
  let i = 1;
  if (status) {
    clauses.push(`trades.status = $${i}`);
    params.push(status);
    i += 1;
  }
  if (marketSlug) {
    clauses.push(`trades.market_slug = $${i}`);
...
// Top-10-most-profitable-traders snapshot — mirrors server/src/db.js's
// replaceWhaleTopTraders/getStoredWhaleTopTraders. fetched_at is repeated
// on every row (same denormalized-snapshot shape as whale_positions above)
// rather than pulled out into a separate "last scan" table.
export async function replaceWhaleTopTraders(pool, { topTraders, fetchedAt }) {
  await pool.query("DELETE FROM whale_top_traders");
  if (!topTraders.length) return;

  const cols = 6;
  const valuesSql = topTraders.map((_, i) => {
    const base = i * cols;
    const placeholders = Array.from({ length: cols }, (_, j) => `$${base + j + 1}`);
    return `(${placeholders.join(", ")})`;
  });
  const params = topTraders.flatMap((t) => [
    t.rank,
    t.wallet ?? null,
    t.name ?? null,
    t.pnl ?? null,
...
export async function getStoredWhaleTopTraders(pool) {
  const { rows } = await pool.query("SELECT * FROM whale_top_traders ORDER BY rank ASC");
  if (!rows.length) return null;
  return {
