...
function buildMarketFilters({ search, status, minVolume, minPrice, maxPrice, tag, hasTrade }, startIndex = 1) {
  const clauses = [];
  const params = [];
  let i = startIndex;
  if (search) {
...
export async function queryMarketsGrouped(
  pool,
  { search, status, sortBy = "volume", minVolume = 0, minPrice, maxPrice, tag, hasTrade, page = 1, pageSize = 25 } = {}
) {
