...
// This is a separate, shorter-lived thing from the DB snapshot below: this
// cache exists purely to avoid redundant external calls within a warm
// process; the DB row is the durable "last known good" copy.
...
export async function getWhaleSlugSet(db, opts) {
  try {
    const { positions } = await fetchWhalePositions(db, opts);
...
export async function getWhalePositionsForSlug(db, slug, opts) {
  try {
    const { positions } = await fetchWhalePositions(db, opts);
