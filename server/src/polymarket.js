...
// used by unattended background work (the sync worker queue, the bulk
// resolution sweep) — pass a lower value (see fetchMarketBySlug) for a
// latency-sensitive, user-initiated call that should fail fast instead of
// making someone watch a spinner through several retries' worth of backoff.
 ...
 * instead of paging through the whole catalog, and by the single-trade
 * "Resolve" button (pass `{ retries: 1 }` there — see getJson). */
